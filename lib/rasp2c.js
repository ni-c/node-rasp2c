 ...
 * @param mode A mode to pass for reading, defaults to 'i'
 * @param callback The callback to call after execution
 */
exports.dump = function(device, range, mode, callback) {
	if (typeof mode === 'function') {
		callback = mode;
	}
	var _mode = typeof mode !== 'function' ? mode : 'i';
	var cmd = 'i2cdump -y ' + ( range ? '-r ' + range + ' ' : '') + i2cbus + ' ' + device + ' ' + _mode;
	exec(cmd, function(err, stdout, stderr) {
		if(err) {
			callback(err);
		} else {
 ...
 * @param callback The callback to call after execution
 */
exports.set = function(device, address, value, mode, callback) {
	var cmd = 'i2cset -y ' 
    + i2cbus
    + ' ' + device
    + ' ' + address
